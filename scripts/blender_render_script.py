...
import sys
from mathutils import Vector # For handling 3D vectors
import argparse
import numpy as np
import math
import os
import pickle
from decimal import getcontext # For high precision decimal operations

# Set high precision for division calculations
getcontext().prec = 28

parser = argparse.ArgumentParser()
parser.add_argument('--object_path_pkl', type = str, required = True) # Path to the .pkl file with object data
parser.add_argument("--parent_dir", type = str, default='./assets')

# Parse arguments passed after the `--` marker
argv = sys.argv[sys.argv.index("--") + 1 :]
args = parser.parse_args(argv)

# Load 3D object paths from the provided .pkl file
uid_paths = pickle.load(open(args.object_path_pkl, 'rb'))

# Set rendering engine to Cycles
bpy.context.scene.render.engine = 'CYCLES'
# Set samples for faster rendering (can be increased for better quality)
bpy.context.scene.cycles.samples = 16
# Set device type to CUDA (GPU)
...
for d in bpy.context.preferences.addons["cycles"].preferences.devices:
    if 'NVIDIA' in d['name']:
        d["use"] = 1 # Use NVIDIA GPU for rendering
        print(d["name"], d["use"])
    else:
        d["use"] = 0 # Disable other devices
        print(d["name"], d["use"])
...
def normalize_and_center_objects(mesh_objects, normalization_range):
    ...
    # Normalize object scale based on the largest bounding box dimension
...
for i in range(8):
    os.makedirs(os.path.join(args.parent_dir, 'rendered_imgs', 'rendered_imgs_view%d'%i), exist_ok=True)
...
for uid_path in uid_paths:
    ...
    for camera_opt in range(-1, 8):
        ...
        if camera_opt == -1:
            ...
            img_path = os.path.join(args.parent_dir, 'rendered_imgs', 'rendered_imgs_view_bg', '%s_bg.png'%(uid_path.split('/')[-1].split('.')[0]))
        ...
        if camera_opt == -1:
            file_path = os.path.join(args.parent_dir, 'rendered_imgs', 'rendered_imgs_view_bg', '%s_bg.png'%(uid_path.split('/')[-1].split('.')[0]))
            bpy.context.scene.render.filepath = file_path
            if os.path.exists(file_path):
               continue
        else:
            file_path = os.path.join(args.parent_dir, 'rendered_imgs', 'rendered_imgs_view%d'%camera_opt, '%s_%d.png'%(uid_path.split('/')[-1].split('.')[0], camera_opt))
            bpy.context.scene.render.filepath = file_path
            if os.path.exists(file_path):
               continue
...
command = "C:\'Program Files'\'Blender Foundation'\'Blender 4.2'\blender -b -P .\scripts\blender_render_script.py\
-- --object_path_pkl .\scripts\assets\example_object_path_multiple_examples.pkl"